...
def load_pretrained_weights(
    ...
    if os.path.isfile(pretrained_weights):
        ...
        if url is not None:
            ...
            print(
                "Supervised weights found at {} and loaded with msg: {}".format(
                    url, msg
                )
            )
            return
...
class PCA:
    ...
    def apply(self, x):
        ...
        if self.mean is not None:
            x -= torch.Tensor(self.mean, device=x.device)
        return torch.mm(torch.Tensor(self.dvt, device=x.device), x.transpose(0, 1)).transpose(0, 1)
