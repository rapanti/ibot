...
def eval_linear(args):
    ...
    def make_train_sample(sample):
        image = sample[".jpg"]
        label = sample[".cls"]
        return train_transform(image), label

    def make_val_sample(sample):
        image = sample[".jpg"]
        label = sample[".cls"]
        return val_transform(image), label

    traindir = os.path.join(args.data_path, "train", "imagenet_train.json")
    valdir = os.path.join(args.data_path, "valid", "imagenet_valid.json")
    dataset_train = wids.ShardListDataset(
        traindir,
        cache_dir="/tmp/train",
        keep=True,
    )
    dataset_train.add_transform(make_train_sample)
    sampler = wids.DistributedChunkedSampler(
        dataset_train, chunksize=1000, shuffle=True, shufflefirst=True, seed=args.seed
    )

    dataset_val = wids.ShardListDataset(
        valdir,
        cache_dir="/tmp/val",
        keep=True,
    )
    dataset_val.add_transform(make_val_sample)
    ...
    if "swin" in args.arch:
        args.patch_size = 4
        model = models.__dict__[args.arch](window_size=args.window_size, patch_size=args.patch_size, num_classes=0)
        embed_dim = model.num_features
    else:
        model = models.__dict__[args.arch](
            patch_size=args.patch_size, num_classes=0, use_mean_pooling=args.avgpool_patchtokens == 1
        )
    ...
    if "swin" in args.arch:
        ...
        feat_dim = embed_dim * (
            args.n_last_blocks * int(args.avgpool_patchtokens != 1) + int(args.avgpool_patchtokens > 0)
        )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            ...
            test_stats = validate_network(
                val_loader, model, linear_classifier, args.n_last_blocks, args.avgpool_patchtokens, args
            )
            print(
            ...
            if utils.is_main_process() and ((test_stats["acc1"] >= best_acc) or epoch == args.epochs - 1):
                ...
                torch.save(
                    save_dict, os.path.join(args.output_dir, "checkpoint_{}_linear.pth".format(args.checkpoint_key))
                )
...
def train(model, linear_classifier, optimizer, loader, epoch, n, avgpool, args):
    metric_logger = utils.MetricLogger(delimiter=" ", steps_per_epoch=args.steps_per_train_epoch)
...
def validate_network(val_loader, model, linear_classifier, n, avgpool, args):
    linear_classifier.eval()
    metric_logger = utils.MetricLogger(delimiter=" ", steps_per_epoch=args.steps_per_val_epoch)
    header = "Test:"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path", default="/path/to/imagenet/", type=str, help="Please specify path to the ImageNet data."
    )
